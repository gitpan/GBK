...
######################################################################
#
# GBK - Source code filter for GBK script
#
...
# when running under perl
    ...
    if (-e qq{$script.e}) {
        ...
        if (($mtime < (stat($script))            [9]) or
            ($mtime < (stat(abspath('egbk.pl')))[9]) or
            ($mtime < (stat(abspath('Egbk.pm')))[9]) or
            ($mtime < (stat(abspath('GBK.pm'))) [9])
        ) {

            # escape script
            if (system(qq{$^X -S egbk.pl $script > $script.e}) != 0) {
                die "GBK: Can't escape $script\n";
            }
        }
    }
...
By describing, "use GBK;" in your script, jperl55.bat, jperl58.bat and
