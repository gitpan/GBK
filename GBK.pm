...
END
    ...
    if (m/^use Egbk(?:\s+[0-9\.]*)?\s*;$/oxms) {
        ...
        printf(<<'END', $Egbk::VERSION); # require run-time routines version
use FindBin;
use lib $FindBin::Bin;
use Egbk %s;
END
...
BEGIN {
    eval qq{ use GBK::Tk; };
    if ($] >= 5.007) {
        eval qq{ use GBK::Tk::Canvas58;    };
        eval qq{ use GBK::Tk::Entry58;     };
        eval qq{ use GBK::Tk::Text58;      };
        eval qq{ use GBK::Tk::Button;      };
        eval qq{ use GBK::Tk::Checkbutton; };
        eval qq{ use GBK::Tk::Frame;       };
        eval qq{ use GBK::Tk::Label;       };
        eval qq{ use GBK::Tk::Listbox;     };
        eval qq{ use GBK::Tk::Menu;        };
        eval qq{ use GBK::Tk::Menubutton;  };
        eval qq{ use GBK::Tk::Message;     };
        eval qq{ use GBK::Tk::Radiobutton; };
        eval qq{ use GBK::Tk::Scale;       };
        eval qq{ use GBK::Tk::Toplevel;    };
    }
    else {
        eval qq{ use GBK::Tk::Canvas55;    };
        eval qq{ use GBK::Tk::Entry55;     };
        eval qq{ use GBK::Tk::Text55;      };
    }
}
USE_TK
    s/^ (\s* use \s+ Tk [^;]* ; \s*? \n) /$1$use_tk/oxmsg;
